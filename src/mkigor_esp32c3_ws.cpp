 ...
 * @author	Igor Mkprog, mkprogigor@gmail.com	@version	V1.1	@date	03.01.2026
 * 
...
/*	gv_mode =
0 - local(without send data),
1 - normal (send data to ThingSpeak, without sleep)
2 - normal + light sleep,
3 - normal + deep sleep	*/
uint8_t				gv_mode = 3;
...
RTC_DATA_ATTR uint8_t RTCv_sleep_count = 0;
...
void gf_readData() {
	...
	bme6.initGasPointX(0, 350, 100, 20);	/// Init heat set point 0
	bme6.do1Meas();
	delay(1000);
	bme6.initGasPointX(0, 350, 100, (int16_t)gv_stru_tphg.temp1);
	bme6.do1Meas();
	lv_measStart = millis();
...
void gf_send2ts() {
	if (WiFi.status() != WL_CONNECTED) mkistdf_wifiCon(); // if no conection wifi, try 1 more

	if (WiFi.status() == WL_CONNECTED) {
		/// Forming STATUS string for ThingSpeak.com
		char lv_status[18] = "______-rFsFzFFg00";	
		DT_stru_t lv_DT;							/// get time from google.com
		...
		lv_status[12] = mkistdf_byte2char(RTCv_sleep_count >> 4);
		lv_status[13] = mkistdf_byte2char(RTCv_sleep_count);
		lv_status[15] = mkistdf_byte2char(gv_luxGT >> 4);
		lv_status[16] = mkistdf_byte2char(gv_luxGT);
		...
		ThingSpeak.setField(1, gv_stru_tphg.temp1);
		ThingSpeak.setField(2, round(gv_stru_tphg.pres1) );
		ThingSpeak.setField(3, round(gv_stru_tphg.humi1) );
		ThingSpeak.setField(4, gv_stru_tphg.gasr1);
		ThingSpeak.setField(5, float(gv_lux.als1));
		ThingSpeak.setField(6, gv_vbat);
...
void setup() {
	...
	Serial.print("Sleep wakeup cause = ");	Serial.println( esp_sleep_get_wakeup_cause() );
	Serial.print("Sleep reset cause = ");	Serial.println( esp_reset_reason() );
	if (esp_reset_reason() == ESP_RST_DEEPSLEEP) {
		Serial.print("\nWakUp from deep sleep. Count = ");	Serial.println(RTCv_sleep_count);
		RTCv_sleep_count++;
	}
#ifdef DEBUG_EN
	mkistdf_cpuInfo();
	...
	uint8_t lv_cc;	///	code chip
	Serial.print("Check a bme680 => ");
	lv_cc = bme6.check(0x77);				///	check bmp680 and SW reset
	if (lv_cc == 0) Serial.print("not found, check cables.\n");
	else {
		Serial.print(" found chip code = ");
		mkistdf_prnByte(lv_cc);
		Serial.println();
	}
	bme6.begin(cd_FIL_x2, cd_OS_x16, cd_OS_x16, cd_OS_x16);
#ifdef DEBUG_EN
	for (uint8_t i = 0; i < 10; i++) bme6.initGasPointX(i, 260 + i * 10, 60 + i * 10);	/// Init all 10 heat set point
...
void loop() {
	...
	Serial.print("Ncount (0-255) = ");	Serial.println(RTCv_sleep_count);
	...
	switch (gv_mode) {
	case 0:
		Serial.println("Wait for 20 sec...");
		delay(20000);
		break;
	case 1:
		Serial.println("Wait for 20 sec...");
		...
		break;
	case 2:
		gv_sleep_time = 60000000;						///	Light sleep mode time = 60 sec = 1 min
		esp_sleep_enable_timer_wakeup(gv_sleep_time);
		Serial.print("Go to light sleep mode for sec = ");  Serial.println(gv_sleep_time / 1000000);
		delay(200);
		esp_light_sleep_start();
		break;
	case 3:
		gv_sleep_time = 600000000;						///	Deep sleep mode time = 600 sec = 10 min
		esp_sleep_enable_timer_wakeup(gv_sleep_time);
		Serial.print("Go to deep sleep mode for sec = ");  Serial.println(gv_sleep_time / 1000000);
		delay(200);
